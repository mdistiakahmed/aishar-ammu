...
/** Sample week shown on the public preview so the dashboard looks complete before sign-in. */
const DEMO_WEEK = 7;
const DEMO_DAY = 3;
...
export function HomePageForGuest() {
  ...
  return (
    <div className="font-bn -my-1 mx-auto flex h-[calc(100dvh-7.25rem)] w-full max-w-6xl flex-col gap-1.5 overflow-hidden sm:my-0 sm:h-[calc(100dvh-7.75rem)] sm:gap-3 lg:h-auto lg:min-h-[min(720px,calc(100dvh-8rem))] lg:gap-6 lg:overflow-visible">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[1.2fr_1fr_1fr] grid-rows-[minmax(0,1.1fr)_minmax(0,1fr)_minmax(0,1.15fr)] gap-1.5 sm:gap-2.5 lg:grid-cols-[1.25fr_1fr_1fr] lg:grid-rows-[minmax(0,1.05fr)_minmax(0,1fr)_minmax(0,1.15fr)] lg:gap-4 xl:gap-5">
        <GuestDashCard
        ...
        >
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center py-0.5">
            <div className="flex aspect-square h-full max-h-26 w-auto items-center justify-center rounded-full bg-[#fff6f1] shadow-inner ring-4 ring-white sm:max-h-36 lg:max-h-56 lg:ring-[6px]">
        ...
        >
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#f4f0fa] sm:h-11 sm:w-11 lg:h-16 lg:w-16">
            <GuestProduceIllustration produceName={size.produceName} className="h-7 w-7 sm:h-9 sm:w-9 lg:h-14 lg:w-14" />
          </div>
